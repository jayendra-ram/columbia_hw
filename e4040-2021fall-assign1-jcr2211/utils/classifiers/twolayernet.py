...
class TwoLayerNet(object):
    ...
    def loss(self, X, y):
        ...
        ###################################################      
        b1 = self.layer2.backward(dx)
        b2 = self.layer1.backward(b1)
    ...
    def step(self, learning_rate=1e-5, optim='SGD', momentum=0.5):
        ...
        W1,b1 = layer1.params
        W2,b2 = layer2.params
        params[0] -= dW1*learning_rate
        params[1] -= db1*learning_rate
        params[2] -= dW2*learning_rate
        params[3] -= db2*learning_rate
    ...
    def predict(self, X):
        ...
        out1 = self.layer1.feedforward(X)
        out2 = self.layer2.feedforward(out1)
        out3 = np.exp(out2)/np.sum(np.exp(out2))
        e = np.exp(out3)
