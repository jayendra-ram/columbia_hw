...
def conv2d_forward(x, w, b, pad: int, stride):
    """
    ...
    new_height = ((height - filter_height + 2 * pad) // stride) + 1
    new_width = ((width - filter_width + 2 * pad) // stride) + 1
    out = np.zeros((batch, new_height, new_width, num_filters))
    #npad = ((0,0),)
    x_pad = np.pad(x,[(0,),(pad,),(pad,),(0,)])
    _, pad_height, pad_width, _ = x_pad.shape


    for bi in range(batch):
        for f in range(num_filters):
            for hi in range(new_height):
                for wi in range(new_width):
                    his = hi*stride
                    wis = wi*stride
                    box = x_pad[bi,  his:his+filter_height, wis:wis+filter_width, :]
                    out[bi, hi, wi, f] = np.sum(box*w[:,:,:,f]) + b[f]
...
def conv2d_backward(d_top, x, w, b, pad, stride):
    ...
    _, d_top_height, d_top_width, d_top_num_filters = d_top.shape
    db = np.zeros((b.shape))
    for i in range(0, num_filters):
        db[i] = np.sum(d_top[:, :, :, i])

    dw = np.zeros((filter_height, filter_width, channels, num_filters))
    
    for ci in range(channels):
        for f in range(num_filters):
            for hi in range(filter_height):
                for wi in range(filter_width):
                    hit = hi + d_top_height * stride
                    wit = wi + d_top_width * stride
                    dw[hi, wi, ci, f] = np.sum(d_top[:, :, :, f] * x_pad[:, hi:hit,wi:wit, ci])
        ...
        for f in range(channels):
            for hi in range(new_height):
                for wi in range(new_width):
                    his = hi*stride
                    wis = wi*stride
                    box = x[bi,  his:his+pool_size, wis:wis+pool_size, f]
                    out[bi, hi, wi, f] = np.average(box)
    #error correction
    #if stride > min(height,width):
...
def avg_pool_backward(dout, x, pool_size, stride):
    ...
    new_height = ((height - pool_size) // stride) + 1
    new_width = ((width - pool_size) // stride) + 1
    dx = np.zeros((batch, height, width, channels))
    #npad = ((0,0),)
