    ...
    def loss(self, X, y):
        ...
        for i in range(num_layers - 1):
            ...
            if use_bn:
                ...
                gamma, beta = params["bn_gamma_{}".format(i)], params["bn_beta_{}".format(i)]
                x, bn_cache = bn_forward(x, gamma, beta, bn_params[i], "train")
                cache[cache_name] = bn_cache
        ...
        for i in range(num_layers - 1):
            ...
            ###############################################
            if use_bn:
                dx = bn_backward(dx, cache["bn_{}".format(j)])            
            ###############################################
            # END OF BATCH NORMALIZATION                  #
            ###############################################  
