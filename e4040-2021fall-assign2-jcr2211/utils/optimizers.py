...
class AdaGradOptim(Optimizer):
    def __init__(self, model, eps=1e-12):
        """
        Inputs:
        :param model: a neural network class object
    ...
    def step(self, model, learning_rate):
        """
        Implement a one-step AdaGrad update on network's parameters
...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            velocitys[k] = beta2 * velocitys[k] + (1 - beta2) * grads[k] * grads[k]
            mhat = momentums[k] / (1 - beta1 ** t)
            vhat = velocitys[k] / (1 - beta2 ** t)
...
class NadamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            mhat = momentums[k] / (1 - beta1 ** t)
            vhat = velocitys[k] / (1 - beta2 ** t)
            mbar = mhat*beta1 + (1-beta1)*grads[k]
            params[k] -= learning_rate * mbar / (np.sqrt(vhat) + eps)
        ###################################################
        #               END OF YOUR CODE                  #
        ###################################################
