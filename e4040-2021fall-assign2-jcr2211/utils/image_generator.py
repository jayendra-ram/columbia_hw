...
class ImageGenerator(object):
    ...
    def next_batch_gen(self, batch_size, shuffle=True):
        ...
        while True:
            if (bc < N//batch_size):
                bc = bc + 1
                yield (x[bc*batch_size:(bc+1)*batch_size], y[bc*batch_size:(bc+1)*batch_size])
            else:
                shuffle(x)
                bc = 0 
        #######################################################################
    ...
    def show(self, images):
        ...
        #######################################################################
        images[0:16]
        fig = plt.figure(figsize=(4, 4))  # width, height in inches
    ...
    def translate(self, shift_height, shift_width):
        ...
        #                         TODO: YOUR CODE HERE                        #
        #######################################################################
        height_translated = np.roll(self.x, shift_height, axis=1)
        width_translated = np.roll(height_translated, shift_width, axis=2)
    ...
    def rotate(self, angle=0.0):
        ...
        #######################################################################
        self.rotated = angle
        rotated = rotate(self.x,angle,axes=(2,1))
        return rotated 
        #######################################################################
        #                                END TODO                             #
        #######################################################################
    ...
    def add_noise(self, portion, amplitude):
        ...
        #                         TODO: YOUR CODE HERE                        #
        #######################################################################
        self.N_aug = int(self.N * portion)
        sample = np.random.choice(self.N, self.N_aug)
        noised = self.x.copy()
        for s in sample:
            noised[s] = gaussian_filter(self.x[s], sigma=amplitude)
        return noised
        #######################################################################
        #                                END TODO                             #
        #######################################################################
